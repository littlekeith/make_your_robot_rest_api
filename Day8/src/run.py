...
def run_function(suite_test, step_functions, kw_name, kw_args, kw_type):
    ''' 
    ...
    if step_functions:
        ...
        suite_test.keywords.create(
            'run_function_keyword', args=[step_functions, kw_name, kw_args], type=kw_type)
...
def import_libs_by_path(suite, libs_path):
    if not libs_path:
        return
...
setup_numbers = [1 for item in suite_steps if item.get('type') == 'setup']
teardown_numbers = [
    1 for item in suite_steps if item.get('type') == 'teardown']

suite = TestSuite(suite_name)
# suite.resource.imports.library('./mykeywords.py')
