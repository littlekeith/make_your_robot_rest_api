...
def init_suite(api_infos):
    ...
    suite = create_suite(api_infos)
    import_libarary(api_infos, suite)
    import_libararys(api_infos, suite)
    suite_setup = get_setup_step(api_infos)
    suite_teardown = get_teardown_step(api_infos)
    if suite_setup:
...
def get_refrence_apis(api_infos):
    '''
        Get apis of the config node
    '''
    from robot_yaml.datas.yaml_fields import OTHER_APIS

    from robot.api import logger
    check_config_node(api_infos)

    try:
        return api_infos.get(OTHER_APIS)
    except Exception as e:
        raise e
