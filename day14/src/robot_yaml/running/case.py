...
def create_test_keywords(suite, yaml_testcase, method, uri, test_obj=None, index=0):
    ...
    from robot_yaml.running.suite import get_setup_step, get_teardown_step
    ...
    try:
        ...
        test_setup = get_setup_step(yaml_testcase)
        test_tear_down = get_teardown_step(yaml_testcase)

        test = test_obj or tc.create_test(suite, name, tag)
        # create test setup
        if test_setup:
            create_step(test, test_setup, 'setup')
        ...
        # create test teardown
        if test_tear_down:
            create_step(test, test_tear_down, 'teardown')
        return test
...
def create_test_keywords_of_other_apis(suite, path, api_yamls_obj, test_obj=None):
    ...
    try:
        ...
        for index, api_case in enumerate(api_yamls_obj):
            ...
            test = create_test_keywords(
                suite, testcases, method, uri, test_obj, index)

            save_response_value_from_other_apis(test, api_case)

    except Exception as e:
        raise e


def save_response_value_from_other_apis(test, extract_yaml_contents):
    '''
    run_func:
        - 
    ...
    '''
    if not extract_yaml_contents:
        return
    from robot_yaml.model.teststeps import create_step
    create_step(test, extract_yaml_contents)
...
def get_case_headers(case):

    from robot_yaml.datas.yaml_fields import HEADERS

    check_testcases_node(case)

    return case.get(HEADERS)
